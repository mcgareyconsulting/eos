...
- 2026-09-04 · fix · src session-2026-09-04 — **Home was missing the pill.** The field reached it (Home spreads the raw doc and `selectHomeTodos` is a generic filter) but `TodoRow` never typed it and `TodoRowLink` never rendered it — a third to-do surface with its own denser row, missed when the field was plumbed through the tab and the L10. Pill extracted to `components/weekly-focus-pill.tsx` rather than copied: one definition, since three surfaces rendering their own is how the tab and Home end up disagreeing about what a weekly focus looks like (same reasoning that consolidated `BoardColumn` and `EntityViewTabs`). `align-middle` is correct in the To-Dos row's inline flow and simply ignored by Home's flex row, so one component serves both without a variant. **Deliberately not added** to the meeting recap — `RecapItem` is one generic shape shared by rocks/to-dos/issues/headlines, and the recap answers what the meeting produced, not what someone is focusing on this week
- 2026-09-04 · fix · src session-2026-09-04 — row alignment: the pill had borrowed `translate-y-px` from the Lock icon beside it, but an icon has no baseline while 10px uppercase text sets its own, so it sat low against the 14px title; and the due-date / owner columns used `pt-0.5` (centre 10) against a title and icon buttons that are both 24px boxes centring at 12, reading 2px high next to every icon. `align-middle` + `leading-none` on the pill, `pt-1` on the columns — and the same `pt-0.5` fixed in `milestone-todo-row.tsx`, which shares the geometry and sits in the adjacent column on the same page
