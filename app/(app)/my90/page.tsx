  ...
  return (
    <span
      className={
        ...
        (isMine ? "text-zinc-900 font-medium" : "text-zinc-500")
      }
    >
      {isMine ? "You" : name}
    </span>
...
function TeamLabel({ name }: { name: string }) {
  if (!name) return null;
  return (
    <span className="hidden md:inline-flex items-center text-xs text-zinc-400 whitespace-nowrap">
      {name}
    </span>
  );
}
...
function DueLabel({ due }: { due: string | null }) {
  if (!due) return <span className="text-xs text-zinc-400 w-24 text-right">—</span>;
