...
import { setIssueFollowing } from "./actions";
import {
  PRIORITY_BADGE,
  PRIORITY_LABEL,
  STATUS_BADGE,
...
export function IssueDetailTrigger({
  ...
  className,
  children,
  /** Deep-linked from a notification (`?issue=`): open on arrival. */
  defaultOpen = false,
}: {
...
/**
 * The detail itself — header, facts, description, Follow and comments —
 * without a shell, so the Issues tab's modal and the notification hub's
 * peek can show the same thing in different frames.
 */
export function IssueDetailBody({
  ...
  const followerIds = issue.follower_ids ?? [];
  ...
  return (
    <div className={cn("space-y-6", className)}>
      ...
      <FollowButton
        entityType="issue"
        following={followerIds.includes(userId)}
        followerCount={followerIds.length}
        toggle={(next) => setIssueFollowing(teamId, issue.id, next)}
      />
